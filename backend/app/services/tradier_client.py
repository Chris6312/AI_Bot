...
class TradierClient:
    ...
    def _request_json(
        ...
        *,
        ...
        if not creds['api_key'] or not creds['account_id']:
            raise RuntimeError(f'Tradier {mode or "PAPER"} credentials are not configured.')
    ...
    def get_quote_sync(self, ticker: str, mode: str | None = None) -> dict[str, Any]:
        return self.get_quotes_sync([ticker], mode=mode).get(ticker.upper(), {})
    ...
    def place_order_sync(self, ticker: str, qty: int, side: str, mode: str | None = None) -> dict[str, Any]:
        creds = self._credentials_for_mode(mode)
    ...
    async def place_order_async(self, ticker: str, qty: int, side: str, mode: str | None = None) -> dict[str, Any]:
        loop = asyncio.get_running_loop()
        return await loop.run_in_executor(None, self.place_order_sync, ticker, qty, side, mode)
    ...
    def get_account_snapshot(self, mode: str | None = None) -> dict[str, Any]:
        ...
        unrealized_pnl = _coalesce_numeric(balances, ['unrealized_pnl', 'unrealized_gain_loss'])
    ...
    def get_positions_snapshot(self, mode: str | None = None) -> list[dict[str, Any]]:
        ...
        for raw_position in raw_positions:
            ...
            quantity = abs(
                _coalesce_numeric(raw_position, ['quantity', 'qty', 'shares', 'share_quantity'])
            )
            if quantity <= 0:
                continue
